...
<td width="300">用户名是:<a href="getUserin?id=<%=us.get(i).getId() %>"><%=us.get(i).getUsername() %></a></td>
...
<td width="180"><a href="SetQ?competence2=3&id=<%=us.get(i).getId()%>" onclick="return ck();"> 取消授权市级管理员</a></td>
...
<td><input type="text" name="competence2" value="3" style="display: none;"/></td>
<td width="180"><a href="SetQ?competence2=3&id=<%=us.get(i).getId()%>" onclick="return ck();"> 取消授权省级管理员</a></td>

</tr>
</table>
</form>
<%} %>
<% if(us.get(i).getCompetence()==4){%>
...
<td width="300">教授</td>
<td><input type="text" name="competence2" value="3" style="display: none;"/></td>
<td width="180"><a href="SetQ?competence2=3&id=<%=us.get(i).getId()%>" onclick="return ck();"> 取消授权为教授</a></td>
...
<%
     ...
     %>
		...
		<br>共<%=n%>页</br> <br>第<%=currentPage%>页</br> <a
			href="getUseradmin?currentPage=1">首页</a> <a
			href="getUseradmin?currentPage=<%=currentPage-1%>">上一页</a> <a
			href="getUseradmin?currentPage=<%=currentPage+1%>">下一页</a> <a
			href="getUseradmin?currentPage=<%=n%>">末页</a>
			...
			%>
			<a href="getUseradmin?currentPage=<%=s%>"><%=s%>&nbsp&nbsp</a> 
			<%
			if(s%50==0){
			    %>
			    <br>
